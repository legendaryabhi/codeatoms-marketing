...
export default function CategoryPage(): React.JSX.Element {
    ...
    // Wait for router to be ready before checking for category
    if (!router.isReady) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-purple-700 via-blue-600 to-indigo-800 text-white flex items-center justify-center">
                <div>Loading...</div>
            </div>
        );
    }
    ...
    const categoryKeywords = `${categoryData.title.toLowerCase()}, developer marketing, ${categoryData.slug}, dev tools marketing, developer relations`;
                ...
                <header className="py-4 sm:py-6">
                    <nav className="flex items-center justify-between">
                        ...
                        <Button
                            variant="ghost"
                            onClick={() => router.push('/')}
                            className="text-white hover:text-yellow-300 hover:bg-white/10 transition-colors text-sm"
                        >
